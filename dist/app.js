...
function selectPlace(name){selectedMapPlace=places.find(p=>p.name===name);$('#selectedPlace').innerHTML=`<p class="eyebrow dark">Selected destination</p><h2>${selectedMapPlace.name}</h2><p>${selectedMapPlace.countryName} · ${selectedMapPlace.type}</p><div>${selectedMapPlace.tags.map(t=>`<span>${t}</span>`).join('')}</div>`;renderMap()}$$('#mapFilters button').forEach(button=>button.addEventListener('click',()=>{activeRegion=button.dataset.region;$$('#mapFilters button').forEach(x=>x.classList.toggle('selected',x===button));renderMap()}));$('#mapSearch').addEventListener('input',renderMap);renderMap();
