...
const dialog=$('#operatorDialog');$('#operatorButton').addEventListener('click',()=>dialog.showModal());$('.dialog-close').addEventListener('click',()=>dialog.close());$('#operatorForm').addEventListener('submit',e=>{e.preventDefault();dialog.close();$('#toast').textContent='Request captured for the prototype.';showToast();e.target.reset()});
const places=[
...
function selectPlace(name){selectedMapPlace=places.find(p=>p.name===name);$('#selectedPlace').innerHTML=`<p class="eyebrow dark">Selected destination</p><h2>${selectedMapPlace.name}</h2><p>${selectedMapPlace.countryName} · ${selectedMapPlace.type}</p><div>${selectedMapPlace.tags.map(t=>`<span>${t}</span>`).join('')}</div><button class="save-map-place" data-save-place="${selectedMapPlace.name}">♡ Save place</button>`;renderMap()}$$('#mapFilters button').forEach(button=>button.addEventListener('click',()=>{activeRegion=button.dataset.region;$$('#mapFilters button').forEach(x=>x.classList.toggle('selected',x===button));renderMap()}));$('#mapSearch').addEventListener('input',renderMap);renderMap();
