(function(){
  ...
  function createPack(region){const [atlasRegion,mapRegion]=regions[region];return {
    region,schema:SCHEMA,edition:'1.32.0',savedAt:new Date().toISOString(),
    ...
    mapCapability:'Bundled Natural Earth country outlines plus curated destination coordinates. Detailed roads, trails and live search require connectivity.',
    limits:'Static editorial reference only. Live travel advisories, health notices, political conditions, biodiversity results, photographs and detailed map tiles are not included.'
  }}
  ...
  async function render(){const packs=await allPacks(),byRegion=new Map(packs.map(pack=>[pack.region,pack]));cards.forEach(card=>{const region=card.dataset.pack,pack=byRegion.get(region)||createPack(region),saved=!!byRegion.get(region),button=card.querySelector('button'),detail=card.querySelector('small');card.classList.toggle('downloaded',saved);button.textContent=saved?'Remove':'Save guide + map';button.setAttribute('aria-pressed',String(saved));detail.textContent=saved?`${pack.countries.length} countries · ${pack.mapDestinations.length} mapped coordinates · ${sizeLabel(bytes(pack))}`:`${pack.countries.length} countries · ${pack.mapDestinations.length} mapped coordinates · offline outline included`});
